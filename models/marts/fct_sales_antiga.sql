with
    ...
    , sales_reason as (
        ...
        left join reason
            on sales_creditcard.order_pk = reason.order_fk
    )   

    , fct_sales as (
        select 
            md5(concat(order_pk, orderdetail_pk, product_pk )) as sales_sk
            , order_pk
            , product_pk
        ...
        left join aggregated_sales
            on sales_reason.order_pk = aggregated_sales.pk_order
)
